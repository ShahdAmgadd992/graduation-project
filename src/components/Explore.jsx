...
import { toursData, popularDestinations, cityCoordinates } from "../data/exploreData";
...
const travelExperiences = ["Heritage", "Adventure", "Coastal", "Marine", "Wellness"];
...
const momentImages = [heritageImg1, heritageImg2, heritageImg3, heritageImg4, heritageImg5, heritageImg6];
  ...
  return (
    <div className="tour-card">
      <div className="tour-card-img-wrap">
        ...
        <button className={`fav-btn ${liked ? "liked" : ""}`} onClick={() => setLiked(!liked)}>
      ...
      <div className="tour-card-body">
        ...
        <div className="tour-card-footer">
          <span className="tour-card-price">
            Start from : <strong style={{ color: "#5596fe" }}>{tour.price}$</strong>
          </span>
        </div>
      </div>
    </div>
...
const Explore = () => {
  ...
  const handleNavigation = (page) => { if (page === "home") window.navigateToHome?.(); };
  const handleSearch = () => { if (searchQuery.trim()) console.log(`Searching: ${searchQuery} in ${selectedCity}`); };
  const handleKeyPress = (e) => { if (e.key === "Enter") handleSearch(); };

  const toggleFilterCity = (city) => {
    setFilterCity((prev) => prev.includes(city) ? prev.filter((c) => c !== city) : [...prev, city]);
  };

  const getFilteredTours = () => {
    let source = selectedExperience ? toursData[selectedExperience] || [] : [];
    if (filterCity.length > 0) source = source.filter((t) => filterCity.includes(t.city));
    if (tourNameSearch) source = source.filter((t) => t.title.toLowerCase().includes(tourNameSearch.toLowerCase()));
    source = source.filter((t) => t.price <= priceRange);
    if (sortBy === "Price: Low to High") source = [...source].sort((a, b) => a.price - b.price);
    else if (sortBy === "Price: High to Low") source = [...source].sort((a, b) => b.price - a.price);
    else if (sortBy === "Rating") source = [...source].sort((a, b) => b.rating - a.rating);
    return source;
  };
  ...
  const getMapCircles = () => {
    ...
    return citiesInTours
      ...
      .map((city) => ({
        ...
        color: selectedExperience ? circleColors[selectedExperience] : circleColors.popular,
      }));
  };

  const mapCircles = getMapCircles();
  const displayedPopular = showMorePopular ? popularDestinations : popularDestinations.slice(0, 6);
  const clearFilter = () => { setSelectedExperience(null); setFilterCity([]); };
        ...
        <div className="explore-content" style={{ marginLeft: "15%" }}>
          ...
          <div className="search-container">
            <div className="custom-select-wrapper" ref={dropdownRef}>
              <div className={`custom-select-trigger ${dropdownOpen ? "open" : ""}`} onClick={() => setDropdownOpen(!dropdownOpen)}>
                <span>{selectedCity}</span>
                <svg className={`chevron ${dropdownOpen ? "rotate" : ""}`} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              {dropdownOpen && (
                <div className="custom-dropdown-menu">
                  {cities.map((city) => (
                    <div key={city} className={`custom-dropdown-item ${selectedCity === city ? "selected" : ""}`} onClick={() => { setSelectedCity(city); setDropdownOpen(false); }}>
                      {city}
                      {selectedCity === city && (
            ...
            <input type="text" className="search-input" placeholder="Search attractions and tours" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyPress={handleKeyPress} />
      ...
      <div className="categories-bar">
        <div className="categories-list">
          {categories.map((cat, index) => (
            <div key={index} className={`category-item ${activeCategory === index ? "active" : ""}`} onClick={() => setActiveCategory(index)}>
        ...
        <aside className="explore-sidebar">
          <div className="sidebar-section">
            ...
            <div className="sidebar-search">
              ...
              <input type="text" placeholder="E.g. Luxor tour" value={tourNameSearch} onChange={(e) => setTourNameSearch(e.target.value)} />
            </div>
          </div>
            ...
            <div className="sidebar-city-list">
              {cities.map((city) => (
                <label key={city} className="sidebar-checkbox">
                  <input type="checkbox" checked={filterCity.includes(city)} onChange={() => toggleFilterCity(city)} />
                  <span>{city}</span>
                </label>
              ))}
            </div>
          ...
          <div className="sidebar-section">
            ...
            {travelExperiences.map((exp) => (
              <label key={exp} className="sidebar-checkbox">
                <input type="checkbox" checked={selectedExperience === exp} onChange={() => setSelectedExperience(selectedExperience === exp ? null : exp)} />
                <span>{exp}</span>
              </label>
            ))}
          </div>
          ...
          <div className="sidebar-section">
            <h4>Price Range</h4>
            <input type="range" min="60" max="2500" value={priceRange} onChange={(e) => setPriceRange(Number(e.target.value))} className="price-slider" />
          ...
          {selectedExperience ? (
            <>
              <div className="cards-header">
                <h2>{filteredTours.length} {selectedExperience} Tours</h2>
                <div className="sort-wrapper" ref={sortDropdownRef}>
                  <span className="sort-label">Sort by :</span>
                  <div className={`sort-trigger ${sortDropdownOpen ? "open" : ""}`} onClick={() => setSortDropdownOpen(!sortDropdownOpen)}>
                    <span>{sortBy}</span>
                    <svg className={`chevron ${sortDropdownOpen ? "rotate" : ""}`} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <polyline points="6 9 12 15 18 9" />
                    </svg>
                  </div>
                  {sortDropdownOpen && (
                    <div className="sort-dropdown">
                      {sortOptions.map((opt) => (
                        <div key={opt} className={`sort-option ${sortBy === opt ? "selected" : ""}`} onClick={() => { setSortBy(opt); setSortDropdownOpen(false); }}>
                          {opt}
                        </div>
                      ))}
                    </div>
              ...
              <div className="active-filters">
                <span className="filter-tag">{selectedExperience} <button onClick={clearFilter}>✕</button></span>
            ...
            <>
              <div className="cards-header">
                <h2><span className="text-blue">Popular</span> Destinations</h2>
              </div>
              <div className="tours-grid">
                {displayedPopular.map((tour) => <TourCard key={tour.id} tour={tour} />)}
              </div>
              {!showMorePopular && popularDestinations.length > 6 && (
                <div className="show-more-wrap">
                  <button className="show-more-btn" onClick={() => setShowMorePopular(true)}>Show More</button>
                </div>
              )}
            </>
          )}
