...
const TripResult = ({ tripPlan, user }) => {
  ...
  const handleUpdateItinerary = async () => {
    ...
    try {
      ...
      const editPayload = {
        ...
        conversation: [
          // required by /ai/edit — must use "user"/"assistant" roles
  ...
  // قبل الـ createTrip، احسبي الـ status

  // ── save trip ─────────────────────────────────────────────────────────────
  // TripResult.jsx — handleSaveTrip
  const handleSaveTrip = async () => {
    setSaveLoading(true);
    setSaveError("");
    try {
      const now = new Date();
      const endDate = tripPlan.endDate ? new Date(tripPlan.endDate) : null;
      const status = endDate && endDate < now ? 3 : 1;

      // ابني الـ plan كـ object { day1: {morning:[], afternoon:[], evening:[]}, ... }
      const planObj = {};
      Object.entries(dayDetails).forEach(([dayNum, slots]) => {
        planObj[`day${dayNum}`] = {
          morning: slots[0]?.rawItems ?? [],
          afternoon: slots[1]?.rawItems ?? [],
          evening: slots[2]?.rawItems ?? [],
        };
      });

      await tripService.createTrip({
        title: `${tripPlan.destination} Trip`,
        destinationGovernorate: tripPlan.destination,
        city: tripPlan.destination,
        startDate: tripPlan.startDate ?? new Date().toISOString(),
        endDate: tripPlan.endDate ?? new Date().toISOString(),
        people: Math.max(1, (tripPlan.adults ?? 1) + (tripPlan.children ?? 0)),
        totalBudgetEgp:
          (tripPlan.budget ?? 0) *
          Math.max(1, (tripPlan.adults ?? 1) + (tripPlan.children ?? 0)),
        totalCost: 0,
        plan: planObj,
        collected: tripPlan.collected ?? {},
        sessionId: tripPlan.requestPayload?.sessionId ?? "",
        isPublic: false,
        status: status, // ← ديناميك مش hardcoded
      });
